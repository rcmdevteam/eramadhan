<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class AddColumnsToRamadhanTransactionsTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::table('ramadhan_transactions', function (Blueprint $table) {
            $table->string('toyyibpay_refno')->nullable();
            $table->string('toyyibpay_billcode')->nullable();
            $table->datetime('mark_as_paid')->nullable();
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::table('ramadhan_transactions', function (Blueprint $table) {
            $table->dropColumn('toyyibpay_refno');
            $table->dropColumn('toyyibpay_billcode');
            $table->dropColumn('mark_as_paid');
        });
    }
}